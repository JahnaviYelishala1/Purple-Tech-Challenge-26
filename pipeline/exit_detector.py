...
def is_exit_crossing(previous_side: str | None, current_side: str) -> bool:
    """Return True when a tracked person moves from right to left across the line."""

    return previous_side == "right" and current_side == "left"


def draw_overlays(frame, line_x: int, exit_count: int, fps_value: float) -> None:
    cv2.line(frame, (line_x, 0), (line_x, frame.shape[0]), (255, 165, 0), 2)
    cv2.putText(
        frame,
        f"FPS: {fps_value:.1f}",
        (15, 30),
...
def detect_exits(
    ...
    try:
        while True:
            ...
            draw_overlays(frame=frame, line_x=line_x, exit_count=stats.exits_detected, fps_value=smoothed_fps)
            frame = safe_resize(frame, max_width=max_width)

            if display_enabled:
                try:
                    cv2.imshow("EXIT Detector", frame)
...
def main() -> int:
    ...
    print(f"Exits detected: {stats.exits_detected}")
    return 0
